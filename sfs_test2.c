 ...
 * Modified by Yanis Jallouli post-course.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
main(int argc, char **argv)
{
  ...
  if (fds[0] >= 0) {
    for (i = 0; i < 100000; i++) {
      ...
      if (x != sizeof(fixedbuf)) {
        ...
        disk_byte_size = i * (int)sizeof(fixedbuf) + x;
  ...
  // Copy of first fill names[0] section, checking to make sure result is the same
  if(sfs_remove(names[0]) < 0) {
    fprintf(stderr, "Removal of file unsuccesful.\n");
    error_count++;
  }
  printf("Testing if remove deletes all blocks properly in %s.\n", names[0]);

  // Copy of fill file section
  fds[0] = sfs_fopen(names[0]);
  tmp = sfs_fwrite(fds[0], test_str, strlen(test_str));
  if (tmp != strlen(test_str)) {
    fprintf(stderr, "ERROR: Tried to write %d, returned %d\n", 
            (int)strlen(test_str), tmp);
    error_count++;
  ...
  if (fds[0] >= 0) {
    for (i = 0; i < 100000; i++) {
      ...
      x = sfs_fwrite(fds[0], fixedbuf, sizeof(fixedbuf));
      if (x != sizeof(fixedbuf)) {
        int diff = i * (int)sizeof(fixedbuf) + x - disk_byte_size;
        if(diff != 0) {
          fprintf(stderr, "ERROR: Disk size has increased by %d upon calling remove\n", diff);
          error_count++;
        }
        break;
      }
  ...
  // And once more test file removal using the delete method
  printf("Finished remove large file test, now testing delete data in %s\n", names[0]);
  int name0_size = sfs_getfilesize(names[0]);
  fds[0] = sfs_fopen(names[0]); // We closed it in last test
  int bytes_deleted = sfs_fdelete(fds[0], name0_size);
  if(bytes_deleted != name0_size) {
    fprintf(stderr, "ERROR: Not all data deleted, expected %d deleted, got %d.\n", name0_size, bytes_deleted);
    error_count++;
  }
  tmp = sfs_fwrite(fds[0], test_str, strlen(test_str));
  if (tmp != strlen(test_str)) {
  ...
  if (fds[0] >= 0) {
    for (i = 0; i < 100000; i++) {
      ...
      if (x != sizeof(fixedbuf)) {
        int diff = i * (int)sizeof(fixedbuf) + x - disk_byte_size;
        if(diff != 0) {
          fprintf(stderr, "ERROR: Disk size has increased by %d upon calling sfs_fdelete\n", diff);
          error_count++;
        }
        break;
      }
