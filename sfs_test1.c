...
main(int argc, char **argv)
{
  ...
  // Personal check if the double indirect block pointer reads & writes correctly
  fds[0] = sfs_fopen(names[0]);
  char new_data_test[12] = "rodeo_times";
  ...
  // Personal check if remove works correctly
  sfs_remove(names[0]);
  mksfs(0); // Just to make it that much harder
  printf("Testing remove file functionality...\n");
  int fd0 = sfs_fopen(names[0]);
  int fd1 = sfs_fopen("potato");
  int fd2 = sfs_fopen("totato");
  int fd3 = sfs_fopen("motato");
  sfs_fwrite(fd3, new_data_test, sizeof(new_data_test));
  sfs_remove("potato");
  int written = sfs_fwrite(fd0, fixedbuf, sizeof(fixedbuf) - 20);
  if (written != sizeof(fixedbuf) - 20) {
    fprintf(stderr, "ERROR: File not removed properly - overwrote %d bytes\n", written);
    error_count++;
  }
  sfs_fseek(fd0, 0);
  int read = sfs_fread(fd0, fixedbuf + 20, sizeof(fixedbuf));
  if(read != (sizeof(fixedbuf) - 20)) {
    fprintf(stderr, "ERROR: File not removed properly - read %d bytes\n", read);
    error_count++;
  }
  sfs_fclose(fd0);
  sfs_fseek(fd3, 0);
  read = sfs_fread(fd3, fixedbuf, sizeof(fixedbuf));
  if(read != sizeof(new_data_test) || strcmp(fixedbuf, new_data_test)) {
    fprintf(stderr, "ERROR: File did not swap properly in directory\n");
    fprintf(stderr, "Read %d bytes, found %s, expected %s\n", read, fixedbuf, new_data_test);
    error_count++;
  }
  sfs_fclose(fd1);
  sfs_fclose(fd2);
